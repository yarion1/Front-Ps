  ...
  return (
    <>
      <Divpage>
          ...
          <Divcoluna>
            {equipament.images.map((item, index) => (
              <DivImage
                ...
                className={`${
                  String(item) === String(imagePreview)
                    ? "selected"
                    : "notSelected"
                }`}
              >
                <img src={item} alt={`${index}`} style={{ width: "90%" }} />
              </DivImage>
            ))}
          </Divcoluna>
          <Imagemprincipal>
        ...
        <Carddetails>
          <Iitleitem>{equipament.title}</Iitleitem>
          <Infopreco>
          ...
          <CardAviso>
            <Iconverified
              style={{ top: "3em", maxWidth: "20%", Maxheight: "25%" }}
            />
            <Text>Para segurança</Text>
            <Text2>
              Realize pagamentos verificando antecipadamente as informações do
              produto.
            </Text2>
          </CardAviso>
          <Buttonconfirm
